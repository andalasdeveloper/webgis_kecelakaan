...
function angkotradius(){    
    ...
    clearangkot();
      hapusMarkerTerdekat();
        ...
        console.log(rad);
        $.ajax({ 
          url: server+'data/angkotradius.php?lat='+pos.lat+'&lng='+pos.lng+'&rad='+rad, data: "", dataType: 'json', success: function(rows)
          { 
...
function carinamamasjid(){
  if(carimasjid.value==''){
    ...
    $.ajax({ url: server+'data/carimasjid.php?cari_nama='+carimasjid.value, data: "", dataType: 'json', success: function (rows){
      cari_masjid(rows);
    }});
  }
}
...
function carinamaresangkot(){
  if(cariress.value==''){
    ...
    $.ajax({ url: server+'data/cariresangkot.php?cari_nama='+cariress.value, data: "", dataType: 'json', success: function (rows){
...
$.ajax({ url: server+'data/caritglkeg.php?id_worship_place='+mesjidevent, data: "", dataType: 'json', success: function (rows){
    console.log("huuuuu");
      console.log(server+'data/caritglkeg.php?id_worship_place='+mesjidevent);
    for (var i in rows) 
        {   
          var row     = rows[i];
           var latitude  = row.latitude ;
...
function tempatibadah() //tampil digitasi tempat ibadah
    {
            ...
            ti.loadGeoJson(server+'data/masjid.php');
...
function ow() //tampil digitasi objek wisata
    {
            ...
            ow.loadGeoJson(server+'data/ow.php');
  ...
  function caritpkec() //fungsi cari mesjid berdasarkan kecamatan
  {
    ...
    $.ajax({ 
        url: server+'data/kecamatan_tp.php?kecamatan='+kec, data: "", dataType: 'json', success: function(rows)
          { 
            if(rows==null)
            {
...
function carikategori()
  {
   ...
   var kat=id_kategori.value;
    console.log(kat);
    $.ajax({ 
        url: server+'data/carikat.php?id_kategori='+kat, data: "", dataType: 'json', success: function(rows)
          { 
  ...
  function pilihkendaraan() //fungsi cari mesjid berdasarkan kendaraaan yang digunakan
  {
    ...
    $.ajax({ 
        url: server+'data/carikendaraan.php?status='+kend, data: "", dataType: 'json', success: function(rows)
          { 
...
function galeri(a){    
      console.log(a);
    window.open(server+'data/gallery.php?idgallery='+a);    
   }
...
function detailkuliner_infow(id){  //menampilkan Information culinary
  $('#info').empty();
   hapusInfo();
	  ...
	  clearroute();
         $.ajax({ 
      url: server+'data/detailrm.php?cari='+id, data: "", dataType: 'json', success: function(rows)
        { 
          console.log(id);
         for (var i in rows) 
          { 
...
function detailevent(id1, id2, id3){  //menampilkan Information event
  ...
  $('#infoevent').empty();
   hapusInfo();
	  ...
	  console.log("hay");
       $.ajax({ 
      url: server+'data/detailevent.php?cari='+id1+'&time='+id2+'&date='+id3, data: "", dataType: 'json', success: function(rows)
        { 
          console.log(id1);
         for (var i in rows) 
          { 
...
function hotelsekitar(latitude,longitude,rad){ // HOTEL SEKITAR MASJID
	...
	function owsekitar(latitude,longitude,rad){ // OW SEKITAR MASJID
          ...
          $.ajax({url: server+'data/cariow.php?lat='+latitude+'&long='+longitude+'&rad='+rad, data: "", dataType: 'json', success: function(rows){ 
            for (var i in rows){ 
              var row = rows[i];
              var id = row.id;
              var nama = row.name;
...
function fasilitas(){
  ...
  $.ajax({ url: server+'data/fasilitas.php?id='+v, data: "", dataType: 'json', success: function(rows){
    for (var i in rows){
      console.log("hy");
      var row = rows[i];
...
function tampilkatwilayah(){ //fungsi cari berdasarkan wilayah dan kategori
  ...
  $('#hasilcari1').show();
    ...
    console.log(cari);
            $.ajax({ 
            url: server+'data//carikatwilayah.php?id='+b+'&cari='+cari, data: "", dataType: 'json', success: function(rows) 
            { 
...
$('#hasilcari1').show();
  ...
  if (arrayLay==''){
    ...
    $.ajax({ url: server+'data/carifasilitas.php?lay='+arrayLay, data: "", dataType: 'json', success: function(rows){
      console.log(server+'data/carifasilitas.php?lay='+arrayLay);
      if(rows==null)
            {
              alert('Mosque not found');
            }
...
function caritglkeg(){
  if(caritgl.value==''){
    ...
    console.log("hau");
    $.ajax({ url: server+'data/caritglkeg.php?tgl='+caritgl.value, data: "", dataType: 'json', success: function (rows){

      console.log(server+'data/caritglkeg.php?tgl='+caritgl.value);
      caritglk(rows);
    }});
  }
}
...
function tampiljurusan()
  {
   ...
   var jur=jurusan.value;
    console.log(jur);
    $.ajax({ 
        url: server+'data/carijur.php?jur='+jur, data: "", dataType: 'json', success: function(rows)
          { 
    ...
    $('#hasilcari').append("<thead><th>Angkot Code</th><th>Route</th><th>Gallery</th></thead>");
      ...
      hapusMarkerTerdekat();   
        ...
        if (lat1==""||lng1==""||lat2==""||lng2=="") {
          ...
          var url_tujuan = server+'data/track.php?lat1='+lat1+'&lng1='+lng1+'&lat2='+lat2+'&lng2='+lng2+'&rad=300'; 
