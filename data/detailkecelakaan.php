...
$querysearch = "SELECT kecelakaan.id_kecelakaan AS id, kecelakaan.total_kerugian AS total_kerugian, kecelakaan.keterangan_lokasi AS keterangan_lokasi,kecelakaan.waktu_kejadian, kecelakaan.image AS image, ST_X(ST_Centroid(kecelakaan.geom)) AS lng, ST_Y(ST_CENTROID(kecelakaan.geom)) AS lat FROM kecelakaan 
    JOIN jenis_kecelakaan on kecelakaan.id_detail=jenis_kecelakaan.id_jenis    
...
while($row = pg_fetch_array($hasil))
    
	{
		  ...
		  $longitude=$row['lng'];
		  $latitude=$row['lat'];
		  $dataarray[]=array('id'=>$id,'total_kerugian'=>$total_kerugian,'waktu'=>$waktu,'keterangan_lokasi'=>$keterangan_lokasi,
